case_fatality_rate_per_country <- function(data = cleaned) {
  ...
  cfr_per_country <- data |>
    ...
    group_by(COUNTRY) |>
    summarise(
      deaths = sum(FDEAD == "Y"), # Patients who died after 6 months
      total_cases = n(),
  ...
  quantiles <- quantile(cfr_per_country$cfr_rate,
...
alternative_cfr_rates <- function(data = cleaned) {
  ...
  german_country_names <- tibble(
    ...
    COUNTRY_GERMAN = c(
      "Albanien", "Argentinien", "Australien", "Österreich", "Belgien",
      "Brasilien", "Bulgarien", "Kanada", "Chile", "Tschechische Republik",
      "Dänemark", "Irland", "Finnland", "Frankreich", "Georgien", "Deutschland",
      "Griechenland", "Hongkong", "Ungarn", "Indien", "Indonesien", "Israel",
      "Italien", "Japan", "Lettland", "Malaysia", "Niederlande", "Neuseeland",
      "Norwegen", "Polen", "Portugal", "Rumänien", "Singapur", "Slowakei",
      "Slowenien", "Südafrika", "Spanien", "Sri Lanka", "Schweden", "Schweiz",
      "Thailand", "Türkei", "Vereinigtes Königreich", "Vereinigte Staaten"
    )
  )

  # Creating a table with the proportion of people who have died of stroke
  # after 6 months
  ...
  cfr_per_country <- data |>
    ...
    group_by(COUNTRY) |>
    summarise(
      deaths = sum(FDEAD == "Y"), # Patients that dies after 6 months of stroke
      total_cases = n(),
      cfr_rate = deaths / total_cases
    ) |>
    arrange(desc(cfr_rate))
